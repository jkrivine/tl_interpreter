(* We keep the `Env` prefix so you can clearly see what's provided by `env.ml`,
   but remove the prefixes for `let*` and `>>` so things are not too ugly. *)
let (let*) = Env.(let*)
let (>>) = Env.(>>)
...
module Contract = struct
  (* `data ?init ?pp name` declares a new data storage location.
     ...
     (It is also possible to initialize the data when the contract is
     instantiated, using `data_set`)
   ...
   *)
  let counter =
    Env.data ~init:0 ~pp:Format.pp_print_int "counter"
  ...
  let construct =
    ...
    Env.code_set incr begin fun () ->
      Env.data_update counter (fun c -> c+1)
    end >>

    (* Note the `>>` which acts as a semicolon between statements. *)

    Env.code_set decr begin fun () ->
      Env.data_update counter (fun c -> c-1)
    end >>

    Env.code_set hello begin fun () ->
      (* echo is the within-blockchain version of 'print' *)
      Env.echo "Hello from Contract"
    end
end
