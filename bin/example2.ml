...
module C1 = struct
  ...
  let construct auth_code =
    Env.code_set say_phrase (fun () ->
        let* phrase_str = Env.data_get phrase in
        Env.echo phrase_str) >>

    Env.code_set set_phrase (fun new_phrase_str ->
...
let () = ignore(Env.execute(
    ...
    let* c11 = Env.tx_create userA "c1.1" (C1.construct auth_code1) in
    let auth_code2 = fun () ->
      Env.return () in
    let* c12 = Env.tx_create userA "c1.2" (C1.construct auth_code2) in
